document.addEventListener("DOMContentLoaded", function () {
  ...
  expenseForm.addEventListener("submit", function (event) {
    ...
    const expenseDate = document.getElementById("expenseDate").value; // Ambil tanggal pengeluaran
    ...
    const expense = { timestamp, expenseDate, expenseType, expenseAmount, expenseNote };
  ...
  // Fungsi untuk menambahkan pengeluaran ke tabel dengan kolom baru "Tanggal Pengeluaran"
  function addExpenseToTable(expense, index) {
    const row = document.createElement("tr");

    // Format tanggal pengeluaran
    const formattedDate = new Date(expense.expenseDate).toLocaleDateString("id-ID", {
      year: "numeric",
      month: "long",
      day: "numeric",
    });
    ...
    row.innerHTML = `
        ...
        <td>${formattedDate}</td> <!-- Tanggal Pengeluaran -->
  ...
  function sendWhatsAppMessage() {
    ...
    expenses.forEach((expense, index) => {
      ...
      message += `============================%0A`;
      message += `Total Pengeluaran: Rp ${expenses.reduce((total, item) => total + item.expenseAmount, 0).toLocaleString("id-ID")}%0A`;
    });

    // Tambahkan pembatas dan pesan Instagram di bagian bawah
    message += `============================%0A`;
    message += `Jangan Lupa Follow Instagram Creator:%0A`;
    message += `www.instagram.com/angga.2864`;
